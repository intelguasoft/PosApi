...
public class EmployeeLinks : IEmployeeLinks
{
	...
	public LinkResponse TryGenerateLinks(IEnumerable<EmployeeDto> employeesDto, string fields, int companyId, HttpContext httpContext)
	{
	...
	private List<DataShapedEntity> ShapeData(IEnumerable<EmployeeDto> employeesDto, string fields) =>
		_dataShaper.ShapeData(employeesDto, fields)
			.Select(e => e.DataShapedEntity)
			.ToList();
	...
	private LinkResponse ReturnShapedEmployees(List<DataShapedEntity> shapedEmployees) =>
	...
	private LinkResponse ReturnLinkdedEmployees(IEnumerable<EmployeeDto> employeesDto,
		string fields, int companyId, HttpContext httpContext, List<DataShapedEntity> shapedEmployees)
	{
		var employeeDtoList = employeesDto.ToList();

		for (var index = 0; index < employeeDtoList.Count(); index++)
		{
			var employeeLinks = CreateLinksForEmployee(httpContext, companyId, employeeDtoList[index].EmployeeId, fields);
			shapedEmployees[index].Add("Links", employeeLinks);
		}

		var employeeCollection = new LinkCollectionWrapper<DataShapedEntity>(shapedEmployees);
	...
	private LinkCollectionWrapper<DataShapedEntity> CreateLinksForEmployees(HttpContext httpContext, LinkCollectionWrapper<DataShapedEntity> employeesWrapper)
	{
